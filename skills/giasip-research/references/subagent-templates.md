...
- `evidence`: when `quote`, **one contiguous verbatim span copied out of the text you actually fetched** (source's own language, ≥8 consecutive words, or ≥15 chars for CJK) — **never retyped from memory**; when `locator`, a table row / PDF page / registry field / patent metadata **plus the reason the source isn't quotable** (scanned PDF, no text layer, paywalled stub). Locator is the fallback for genuinely unquotable sources, not a shortcut for "didn't open it" — fabricating a quote is the worse failure, which is why locator stays legal
  - **One span, unedited.** The quote must be reproducible by selecting an unbroken run of characters in the source and copying it. That rules out, structurally: elision of any spelling (`...`, `…`, `[…]`, `[omitted]`, `--snip--`), concatenating two spans (with `/`, `·`, or anything else), and appending your own annotation (`(Recital 31)`, `(full text in snapshot)`) inside the field. Need two separated passages? That is **two claims**, or one claim quoting the single passage that actually carries it. The rule is not "no ellipsis" — banning one spelling of omission just moves it; the invariant is *contiguity*, and every form of splicing violates it
  - **Copy from the snapshot, not from the page.** The gate matches against `snapshots/<claim_id>.txt`, which holds the **normalized** body. Normalization strips markup, and markup is where superscripts and tag-adjacent punctuation live: `10<span class="oj-super">25</span>.` normalizes to `10 25 .`, so a quote reading `10^25.` — faithful to what a human sees, and typed in good faith — can never match its own snapshot. Empirically this hits exactly the highest-stakes claims (FLOP thresholds, statutory dates). Write the snapshot first, then take the quote out of that file
- `retrieved_at` / `source_sha256`: ISO8601 fetch time + `sha256(normalized main text)[:16]` **at that moment**. These are what let a later re-check tell **"the quote was wrong"** apart from **"the page changed since"** — without them every re-check conflates the two.
  ...
  - **Persist the snapshot with the hash** for every `central` / high-risk claim: write the normalized main text — **the exact bytes you hashed, already normalized, not the raw body** — to the run's `snapshots/<claim_id>.txt`; a file that still needs normalizing proves the hash was taken from something other than what is on disk, and the quote gate rejects it. A hash whose text was never kept can be recomputed by nobody, so a later re-check has to take your word for it — which is the whole thing the anchor exists to avoid. Use whichever reader actually yields the full body for that format; the snapshot, not the tool, is what is required.
